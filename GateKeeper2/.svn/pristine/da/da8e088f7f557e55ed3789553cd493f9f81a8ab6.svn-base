...
Public Class regControl
    ...
    Public Function readReg(ByVal path As String)
        'Utils.WriteToEventLog("Reading " & path,EventLogEntryType.Information)
        Dim parray() As String
        Dim regkey As RegistryKey
        parray = path.Split("#")
        Try
            regkey = getRegKey(path, False)
            'Utils.WriteToEventLog("Read: " & regkey.GetValue(parray(2)).ToString,EventLogEntryType.Information)
            Return regkey.GetValue(parray(2)).ToString
        Catch ex As Exception
            'Utils.WriteToEventLog("readReg exception - " & ex.Message,EventLogEntryType.Error)
            Return "FAIL"
        End Try
    End Function
    ...
    Public Sub addReg(ByVal path As String, ByVal value As String, ByVal type As String)
        ...
        regkey.Flush()
        Catch ex As Exception
            Utils.WriteToEventLog("Regcontrol:addReg(" & path & "," & value & "," & type & ")",EventLogEntryType.Error)
            Utils.WriteToEventLog(ex.Message,EventLogEntryType.Error)
        End Try
    End Sub
    ...
    Private Function createHKLM(ByVal path)
        ...
        For Each pelement As String In parray
            ...
            If regKey Is Nothing Then
                regKey = Registry.LocalMachine.OpenSubKey(oldPath, True)
                regKey = regKey.CreateSubKey(pelement)
            End If
    ...
    Private Function createHKCU(ByVal path)
        ...
        For Each pelement As String In parray
            fullPath = fullPath & pelement
            regKey = Registry.Users.OpenSubKey(fullPath, True)
            If regKey Is Nothing Then
    ...
    Private Function getRegKey(ByVal path As String, ByVal create As Boolean)
        ...
        'If depth < 2 Then
            ...
            Catch ex As Exception
                Utils.WriteToEventLog("Error while opening at "& pos & "-" & parray(1) & " - " & ex.Message, EventLogEntryType.Error)
            End Try
        'End If
