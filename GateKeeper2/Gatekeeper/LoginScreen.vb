...
Public Class LoginScreen
    ...
    Private Sub LoginScreen_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            ...
            AUPBox.AppendText(vbCrLf & "If this error recurrs after a reboot please report this error to Network Services" & vbCrLf)
            Utils.WriteToEventLog("Middleman Communications Failure on port " & port, EventLogEntryType.Error)
            Me.Login.Visible = False
            Me.Logout.Text = "Reboot"
            connected = False
            uninstall()
        End Try
        If connected Then
    ...
    Private Sub uninstall()
        Try
            If Not Directory.Exists("c:\programdata\ashby school\") Then
                Directory.CreateDirectory("C:\programdata\ashby school")
            End If

            File.Create("C:\programdata\ashby school\reinstall.dat")
    ...
    Private Sub copyMOTD()
        ...
        End Try
        HideFile(GetFolderPath(SpecialFolder.ApplicationData) & "\Ashby School\" & My.Resources.LocalMOTD)
    End Sub
